...
void gen_action_array_h(Config *c, FILE *fp) {
    ...
    for (int i = 0; i < array_size(c->passes); ++i) {
        ...
        out_statement("void *dispatch_pass_%s(void *, NodeType)",
                      pass->id->lwr);
    }
...
void gen_pass_dispatchers(Config *config, Pass *pass, int indent, FILE *fp) {
                   ...
                   pass->id->lwr);
    {
        ...
        out_begin_case("NT_%s", config->root_node->id->lwr);
        if (pass->func) {
            out_statement("return %s(node)", pass->func->lwr);
        } else {
            out_statement("return pass_%s_entry(node)", pass->id->lwr);
        }
        out_end_case();
        ...
        for (int i = 0; i < array_size(values); ++i) {
            ...
            if (pass->func) {
                out_statement("return %s_%s(node)", pass->func->lwr, root);
            } else {
                out_statement("return pass_%s_entry_%s(node)", pass->id->lwr,
                              root);
            }
...
void gen_action_array_c(Config *c, FILE *fp) {
    ...
    // the end.
    out_statement("static ccn_action_t action_array[CCN_NUM_OF_ACTIONS + 1] = "
                  "{{.type = action_NULL}}");
    ...
    out_start_func(
        "ccn_action_t *make_pass_action(ccn_action_t* action, void "
    ...
    out_start_func("ccn_action_t *make_phase_action(ccn_action_t* action, enum "
                   "ACTION_IDS id, enum ACTION_IDS *id_table, bool "
                   "(*gate)(void), char *name, bool is_cycle)");
    {
        ...
        out_statement("phase->root_type = NT_%s", c->root_node->id->lwr);
    ...
    out_start_func("void ccn_init_action_array()");
    {
        for (int i = 0; i < array_size(c->traversals); ++i) {
            ...
            out_statement("make_trav_action(&action_array[ACTION_ID_%s], "
                          ...
                          trav->id->lwr, trav->id->lwr, trav->id->lwr,
                          trav->id->lwr);
        }
        ...
        for (int i = 0; i < array_size(c->passes); ++i) {
            ...
            out_statement("make_pass_action(&action_array[ACTION_ID_%s], "
                          ...
                          pass->id->lwr, pass->id->lwr, pass->id->lwr,
                          pass->id->lwr);
        }
        ...
        for (int i = 0; i < array_size(c->phases); ++i) {
            ...
            if (phase->gate_func != NULL) {
                out_statement("make_phase_action(&action_array[ACTION_ID_%s], "
                              ...
                              phase->id->lwr, phase->id->lwr, phase->id->lwr,
                              phase->gate_func, phase->id->lwr, cycle);
            } else {
                out_statement("make_phase_action(&action_array[ACTION_ID_%s], "
            ...
            if (phase->root != NULL) {
                out_statement(
                    "action_array[ACTION_ID_%s].phase->root_type = NT_%s",
                    phase->id->lwr, phase->root);
            }
        }
                   ...
                   "TraversalType trav_type)");
    {
        ...
        for (int i = 0; i < array_size(c->nodes); ++i) {
            ...
            out_begin_case("NT_%s", node->id->lwr);
            out_statement("trav_start_%s(node, trav_type)", node->id->lwr);
            out_statement("break");
            out_end_case();
        }
                   ...
                   "NodeType to)");
    {
        for (int i = 0; i < array_size(c->sub_root_pairs); ++i) {
            ...
            out_statement("return TRAV__CCN_PhaseDriver_Find%sFrom%s", pair->to,
                          pair->from);
            out_end_if();
        }
    }
    ...
    ccn_set_t *to_values_already_processed =
        ccn_set_string_create(array_size(c->sub_root_pairs));
    for (int i = 0; i < array_size(c->sub_root_pairs); ++i) {
        ccn_sub_root_pair *pair = array_get(c->sub_root_pairs, i);
        if (ccn_set_insert(to_values_already_processed, pair->to)) {
            out_begin_if("NT_%s == type",
                         pair->to); // TODO: Check for double entries here in
                                    // the pair->to values.
