...
void subtree_generate_call_find_sub_root(char *root, char *to_find, FILE *fp,
                                         Phase *phase, int *_indent) {
    int indent = *_indent;
    out_statement(
        "trav_start_%s(temp_root, TRAV__CCN_PhaseDriver_Find%sFrom%s)", root,
        to_find, root);
...
void subtree_generate_set_handler(char *root, ccn_set_t *funcs,
    ...
    fp = get_fp(source_file, "a");
    {
        ...
        out("phase_driver->curr_sub_root = mem_alloc(sizeof(struct "
            "ccn_subroot));\n}\n");
...
void subtree_generate_find_traversal(Phase *phase, char *root,
                                     Node *original_root, Config *config,
                                     ccn_set_t *gen, ccn_set_t *funcs) {
    ...
    for (int i = 0; i < array_size(phase->actions); ++i) {
        ...
        if (action->type == ACTION_PHASE) {
            ...
            if (child->root != NULL) {
                subtree_generate_find_traversal(child, child->root, target_root,
                                                config, gen, funcs);
            }
        }
    }
    ...
    Traversal *trav =
        create_traversal(trav_name, NULL, ccn_str_cpy("CCN"), expr, NULL);
...
void subtree_generate_traversals(Config *config) {
    ...
    for (int i = 0; i < array_size(phases); ++i) {
        ...
        if (phase->root == NULL)
            continue;
        subtree_generate_find_traversal(phase, phase->root, config->root_node,
                                        config, generated, generated_funcs);
    }
 ...
 * We could do a void*, but that would lose type safety, so generate type
 * dependant functions.
 */
void generate_mark_functions(Config *config, array *phases) {
    ...
    for (int i = 0; i < array_size(roots); ++i) {
        ...
        out_statement(
            "printf(\"Given root does not match with current top.\\n\")");
