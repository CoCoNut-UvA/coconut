...
void gen_action_array_h(Config *c, FILE *fp) {
    ...
    /// Order is important and should be passes,  traversals and phases.
    {
        ...
        out_enum_field("ACTION_ID_%s", "free");
        size++;
        ...
        out_enum_field("ACTION_ID_%s", "cleanup");
        size++;
    }
...
//             } else {
//                 out_statement("return pass_%s_entry_%s(node)", pass->id,
//                 root);
//             }
...
void gen_action_array_c(Config *c, FILE *fp) {
    ...
    for (int i = 0; i < array_size(c->phases); ++i) {
        ...
        bool is_root = ccn_str_equal(phase->id->orig, c->start_phase->id->orig);

        size_t actions = array_size(phase->actions) + 1 + is_root;
        ...
        if (is_root) {
            out("ACTION_ID_cleanup, \n");
        }
        out("ACTION_ID_NULL, \n");
        out("};\n");
    }

    out("enum ACTION_IDS cleanup_ids_table[2] = { ACTION_ID_free, "
        "ACTION_ID_NULL,\n");
    out("};\n");
    ...
    out("{action_traversal, ACTION_ID_free, \"Free\", .traversal = "
        "{TRAV_free,},},\n");
    ...
    int indent = 0;
    out_start_func(
        "ccn_action_t *get_ccn_action_from_id(enum ACTION_IDS action_id)");
