...
//         actions); for (int j = 0; j < array_size(phase->actions); ++j) {
...
//         out_statement("pass->func->lwr = func");
...
//         out_statement("phase->root_type = NT_%s",
//         c->root->lwr_node->id->lwr); out_statement("action->type =
//         action_phase"); out_statement("action->action_id = id");
...
//                 trav->id->lwr, trav->id->lwr, trav->id->lwr, trav->id->lwr);
//         }
...
//                 pass->id->lwr, pass->id->lwr, pass->id->lwr, pass->id->lwr);
//         }
